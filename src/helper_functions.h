...
inline double multivariant_gaussian(double x, double y, double mu_x, double mu_y, double sigma_x, double sigma_y)
{
  double exponent = pow(x - mu_x, 2.0f) / (2 * pow(sigma_x, 2)) + pow(y - mu_y, 2) / (2 * pow(sigma_y, 2));
  return  1 / (2 * M_PI * sigma_x * sigma_y) * pow(M_E, -exponent);
}
